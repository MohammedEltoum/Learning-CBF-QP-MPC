...
for t = 1:N_sim
    ...
    if uDev >= 0.01
        X = [X; [x'   uOpt'] ];
        y = [y; [h(x) dhdx'*(f(x)+g(x)*uOpt) dhdx'] ];
        count = 0;
    else
        count = count + 1;
    end
...
% scatter3(0,0,0,'r*')
% hold on
% scatter3(Xr(:,1),Xr(:,2),y(:,1), 30, y(:,1), 'filled')

% Xr = X(:,1:2)-X(:,4:5);
% figure(777)
% % scatter3(0,0,0,'r*')
% % hold on
% scatter3(Xr(:,1),Xr(:,2),y(:,1), 30, y(:,1), 'r')
% hold on
% scatter3(Xr(:,1),Xr(:,2),y_pred(:,1), 30, y_pred(:,1), 'b')
